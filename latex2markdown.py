...
class LaTeX2Markdown(object):
    def __init__(self, latex_string, block_configuration = _block_configuration, block_counter = defaultdict(lambda: 1)):
        ...
        self._header_re = re.compile(r"""\\(?P<header_name>chapter|section|subsection)
                                    {(?P<header_contents>.*?)}""", #
                                    flags=re.DOTALL + re.VERBOSE)
    ...
    def _format_block_contents(self, block_name, block_contents):
        line_indent_char = self._block_configuration[block_name]["line_indent_char"]
    ...
    def _format_list_contents(self, block_name, block_contents):
        list_heading = self._block_configuration[block_name]["list_heading"]
...
y = LaTeX2Markdown(test)
print y.latex_to_markdown()